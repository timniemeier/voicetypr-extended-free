...
export function LLMModelsSection() {
  ...
  const activeModelName = isUsingCustomProvider
    ? customModelName
    : getModels(aiSettings.provider).find((m) => m.id === aiSettings.model)?.name ||
      aiSettings.model;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Language-model post-processing providers and API keys
            </p>
          </div>
            ...
            <div className="grid gap-3">
              {AI_PROVIDERS.map((provider) => {
                ...
                const isActive = provider.isCustom
                  ? isCustomActive
                  : Boolean(
                      aiSettings.provider === provider.id && aiSettings.enabled
                    );
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-3">
                    AI formatting automatically improves your transcribed text with
                    proper punctuation, grammar, and style. Edit the prompt itself in
                    the Prompts tab.
                  </p>
                </div>
              </div>
