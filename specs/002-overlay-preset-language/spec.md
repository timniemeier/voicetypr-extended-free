...
- Q: Spec 003 (Settings Tab Restructure) landed first and replaced the legacy `EnhancementOptions.preset` enum with a string `active_prompt_id` that can point at a built-in OR a user-authored custom prompt. When un-stashing 002, should the cycler keep the old enum surface (Option A — thin shim translating enum ↔ id) or be rewritten to operate on `active_prompt_id` directly (Option B)? → A: **Option B — cycler operates on `active_prompt_id` strings directly.** Recorded per `specs/003-settings-tab-restructure/follow-ups.md` § FU-2. Rationale: removes a permanent ceiling on the cycler (a future feature can widen the cycle to include custom prompts), keeps the data model unified (one canonical "active" surface — no enum-vs-id drift), and the diff cost is one-off versus carrying a shim forever. Scope for 002 stays tight: the cycle ring is **only** the four built-ins in canonical order (`builtin:default → builtin:prompts → builtin:email → builtin:commit → wrap`); custom prompts are intentionally NOT in the cycle for this feature. The backend hotkey handler reads `active_prompt_id` via `get_active_prompt`, advances it via the new pure helper `next_active_prompt_id` in `src-tauri/src/recording/cycle_actions.rs`, persists via `set_active_prompt`, and emits `active-prompt-changed { id, label }` (replaces the never-shipped `active-preset-changed { preset }` payload). The overlay (`RecordingPill`) seeds its label from `get_active_prompt` on mount and stays in sync via the new event; the Prompts tab's `usePromptLibrary` hook also listens for the event so the active-dot stays correct when the cycle is triggered while the tab is open.
